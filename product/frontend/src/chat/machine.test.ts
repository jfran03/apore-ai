...
describe('chatReducer', () => {
  ...
  it('runs the happy path: question -> answer -> graded -> rating -> reflection -> continue -> next question', () => {
    ...
    expect(state.errorRecovery).toBe('awaiting_answer');
    ...
    const beforeRatingCount = state.transcript.length;
    state = chatReducer(state, { type: 'rating_sent', rating: 'ok' });
    expect(state.status).toBe('working');
    expect(state.errorRecovery).toBe('awaiting_rating');
    expect(state.transcript).toHaveLength(beforeRatingCount);
    state = chatReducer(state, {
      type: 'turn_result',
    ...
    const beforeContinueCount = state.transcript.length;
    state = chatReducer(state, { type: 'continue_sent' });
    expect(state.errorRecovery).toBe('reflection');
    expect(state.transcript).toHaveLength(beforeContinueCount);
